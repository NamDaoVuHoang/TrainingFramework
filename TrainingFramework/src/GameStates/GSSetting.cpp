...
void GSSetting::InitBackground(std::shared_ptr<Models> model, std::shared_ptr<Shaders> shader, string tex, int i) {
...
void GSSetting::Init()
{
	...
	//play button
	auto texture = ResourceManagers::GetInstance()->GetTexture("Btn_V11");
	...
	//exit button
	texture = ResourceManagers::GetInstance()->GetTexture("Btn_V11");
	...
	m_Text_gameName = std::make_shared< Text>(shader, font, "SETTING", TEXT_COLOR::BLACK, 1.0);
	m_Text_gameName->Set2DPosition(Vector2(screenWidth / 2 - 80, 120));

	std::shared_ptr<Text> text_button = std::make_shared<Text>(shader, font, "Start Game", TEXT_COLOR::BLACK, 0.9);
	text_button->Set2DPosition(Vector2(screenWidth / 2 - 80, 205));
	m_list_Text_button.push_back(text_button);

	text_button = std::make_shared<Text>(shader, font, "Back", TEXT_COLOR::BLACK, 0.9);
